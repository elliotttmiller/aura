...
class wdDialog(forms.Dialog):
    def __init__(self):
        ...
        # input controls
        self.XCheckBoxGroup, self.XCheckBox = cg.CreateCheckBoxGroup('X Axis: ', self.LabelWidth, True, self.OnFormChanged)  
        self.YCheckBoxGroup, self.YCheckBox = cg.CreateCheckBoxGroup('Y Axis: ', self.LabelWidth, True, self.OnFormChanged)
        self.ZCheckBoxGroup, self.ZCheckBox = cg.CreateCheckBoxGroup('Z Axis: ', self.LabelWidth, True, self.OnFormChanged)
        self.XSliderGroup = cg.CreateSliderGroup('Width : ', self.LabelWidth, 0.0, 5.0, 2, 0.0, self.Solve)    
        self.YSliderGroup = cg.CreateSliderGroup('Length: ', self.LabelWidth, 0.0, 5.0, 2, 0.0, self.Solve)
        self.Z1SliderGroup = cg.CreateSliderGroup('Height: ', self.LabelWidth, 0.0, 5.0, 2, 0.0, self.Solve) 
        self.Z2SliderGroup = cg.CreateSliderGroup('Depth: ', self.LabelWidth, 0.0, 15.0, 2, 0.0, self.Solve) 
    ...
    def LayoutForm(self):
        ...
        self.Layout.AddRow(self.XCheckBoxGroup)
        self.Layout.AddRow(self.YCheckBoxGroup)
        self.Layout.AddRow(self.ZCheckBoxGroup)
        self.Layout.AddRow(cg.CreateVerticalSpacer(5))
        self.Layout.AddRow(cg.CreateHR())
        self.Layout.AddRow(cg.CreateVerticalSpacer(5))
        self.Layout.AddRow(self.XSliderGroup)
        self.Layout.AddRow(self.YSliderGroup)
        self.Layout.AddRow(self.Z1SliderGroup)
        self.Layout.AddRow(self.Z2SliderGroup)
    ...
    def OnFinalizeButtonClick(self, sender, e):
        if len(self.GemIDs) > 0:
            ...
            if not rs.IsLayer('gem axis'):
                rs.AddLayer('gem axis', System.Drawing.Color.White, True, False, None)

            layer = sc.doc.Layers.FindName('gem axis')
            ...
            for i in range(len(self.GemIDs)):
                axis_ids = []
                if len(self.XAxes) > 0:           
                    axis_id = sc.doc.Objects.AddCurve(self.XAxes[i], atts)
                    axis_ids.append(axis_id)
                    all_axes.append(axis_id)

                if len(self.YAxes) > 0:           
                    axis_id = sc.doc.Objects.AddCurve(self.YAxes[i], atts)
                    axis_ids.append(axis_id)
                    all_axes.append(axis_id)

                if len(self.ZAxes) > 0:           
                    axis_id = sc.doc.Objects.AddCurve(self.ZAxes[i], atts)
                    axis_ids.append(axis_id)
                    all_axes.append(axis_id)
                ...
                if len(axis_ids) > 1:
                    grp = rs.AddGroup()
                    rs.AddObjectsToGroup(axis_ids, grp)
                    axis_groups.append(axis_ids)

            if len(all_axes) > 1:
                grp = rs.AddGroup()
                for axis in all_axes:
                    rs.AddObjectsToGroup(axis, grp)
                    
        sc.doc.Views.Redraw()
        self.DisposeObjects(self.BasePlanes)
        self.DisposeObjects(self.XAxes)
        self.DisposeObjects(self.YAxes)
        self.DisposeObjects(self.ZAxes)
    ...
    def SetGems(self, sender):
        ...
        if len(gem_ids) == 0:
            ...
            for gem_id in gem_ids:
                ...
                self.BasePlanes.append(gem_pln)
    ...
    def Solve(self, sender):
        ...
        for i in range(len(self.GemIDs)):
            ...
            gem_pln = self.BasePlanes[i]

            # get import gem distances (from world origin)
            gem = rs.coercebrep(gem_id)
            bbox = gem.GetBoundingBox(gem_pln)
            gem_front = bbox.Min.Y
            gem_back = bbox.Max.Y
            gem_right = bbox.Max.X
            gem_left = bbox.Min.X
            gem_top = bbox.Max.Z
            gem_bottom = bbox.Min.Z

            # get extra distances from sliders
            extra_x = self.XSliderGroup.Value
            extra_y = self.YSliderGroup.Value
            extra_z1 = self.Z1SliderGroup.Value
            extra_z2 = self.Z2SliderGroup.Value           

            # create lines
            xform = rg.Transform.PlaneToPlane(rg.Plane.WorldXY, gem_pln)
            if self.XCheckBox.Checked:
                pnt_right = rg.Point3d(gem_right + extra_x, 0, 0)
                pnt_left = rg.Point3d(gem_left - extra_x, 0, 0)

                x_axis = rg.Line(pnt_left, pnt_right).ToNurbsCurve()
                x_axis.Transform(xform)
                self.XAxes.append(x_axis)

                pnt_right.Transform(xform)
                pnt_left.Transform(xform)
                self.Points.append(pnt_right)
                self.Points.append(pnt_left)

            if self.YCheckBox.Checked:
                pnt_front = rg.Point3d(0, gem_front - extra_y, 0)
                pnt_back = rg.Point3d(0, gem_back + extra_y, 0)
                
                y_axis = rg.Line(pnt_front, pnt_back).ToNurbsCurve()
                y_axis.Transform(xform)
                self.YAxes.append(y_axis)

                pnt_front.Transform(xform)     
                pnt_back.Transform(xform)
                self.Points.append(pnt_front)
                self.Points.append(pnt_back)

            if self.ZCheckBox.Checked:
                pnt_top = rg.Point3d(0, 0, gem_top + extra_z1)
                pnt_bottom = rg.Point3d(0, 0, gem_bottom - extra_z2)

                z_axis = rg.Line(pnt_bottom, pnt_top).ToNurbsCurve()
                z_axis.Transform(xform)
                self.ZAxes.append(z_axis) 

                pnt_top.Transform(xform)
                pnt_bottom.Transform(xform)
                self.Points.append(pnt_top)
                self.Points.append(pnt_bottom)
        ...
        for axis in self.XAxes:
            self.RenderObjects.append([axis, cam.PointColor])   
        for axis in self.YAxes:
            self.RenderObjects.append([axis, cam.PointColor])   
        for axis in self.ZAxes:
            self.RenderObjects.append([axis, cam.PointColor])    
