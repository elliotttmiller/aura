...
class wdDialog(forms.Dialog):
    ...
    def __init__(self):
        ...
        # input controls
        self.DiameterSG = cg.CreateSliderGroup('Diameter: ', self.LabelWidth, 10.0, 30.0, 2, 15.0, self.Solve)
        self.SeamPositionRBLG, self.SeamPositionRBL = cg.CreateRadioButtonListGroup('Seam Position: ', self.LabelWidth, ['Top', 'Bottom'], self.OnFormChanged, default_index = 1)

        # bottom buttons
        self.OKButton = cg.CreateButton('Finalize', self.OnFinalize)
    ...
    def LayoutForm(self):
        ...
        self.Layout.AddRow(self.DiameterSG)
        self.Layout.AddRow(self.SeamPositionRBLG)
        self.Layout.EndVertical()

        self.Layout.BeginVertical()
        self.Layout.AddRow(cg.CreateVerticalSpacer(5))
        self.Layout.AddSpace()
        self.Layout.AddRow(None, self.OKButton, self.CancelButton)
        self.Layout.EndVertical()
    ...
    def OnFinalize(self, sender, e):
        if not rs.IsLayer('sizing circle'):
            rs.AddLayer('sizing circle', System.Drawing.Color.FromArgb(255, 100, 12, 12))
        circle_id = sc.doc.Objects.AddCurve(self.SizingCircle)
        rs.ObjectLayer(circle_id, 'sizing circle')
        rs.ObjectName(circle_id, 'Size: ' + str(self.DiameterSG.Value) + 'mm')
    ...
    def Solve(self, sender):
        ...
        d = self.DiameterSG.Value
        r = d/2

        circle = rg.Circle(rg.Plane.WorldZX, r).ToNurbsCurve()
        if self.SeamPositionRBL.SelectedIndex == 1:
            circle.Rotate(math.radians(180), rg.Plane.WorldZX.ZAxis, rg.Point3d.Origin)
        
        self.SizingCircle = circle
        self.Seam = circle.PointAtStart

        self.RenderObjects.append([self.SizingCircle, cam.CurveColor])
        self.RenderObjects.append([self.Seam, cam.PointColor])
